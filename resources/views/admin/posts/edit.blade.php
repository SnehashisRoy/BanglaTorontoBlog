...
@section('content')
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5 sm:p-8 max-w-2xl">
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900 mb-6">Edit Post</h1>
            ...
            <div>
                ...
                @if($post->image_path)
                    <div class="mb-3 flex items-center gap-3">
                        <img src="{{ $post->imageUrl() }}" alt="" class="h-20 w-20 rounded-lg object-cover border border-gray-200">
                ...
                <p class="mt-1 text-xs text-gray-400">JPG, PNG or WEBP, up to 2MB. Uploading a new image replaces the current one.</p>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="title_en" name="title_en"
                           value="{{ old('title_en', $enTranslation?->title) }}"
                           class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
                </div>
                <div>
                    <label for="body_en" class="block text-sm font-medium text-gray-700 mb-1">Body</label>
                    <textarea id="body_en" name="body_en" rows="8"
                              class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 font-mono">{{ old('body_en', $enTranslation?->body) }}</textarea>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" id="title_bn" name="title_bn"
                           value="{{ old('title_bn', $bnTranslation?->title) }}"
                           class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
                </div>
                <div>
                    ...
                    <textarea id="body_bn" name="body_bn" rows="8"
                              class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 font-mono">{{ old('body_bn', $bnTranslation?->body) }}</textarea>
                </div>
            </div>
